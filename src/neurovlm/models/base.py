...
from neurovlm.models.registry import (
    ModelDomain,
    ModelFamily,
    ModelLoader,
    ModelSpec,
...
class ProjHead(nn.Module):
    ...
    def from_pretrained(model_name: str) -> nn.Module:
        ...
        match model_name:
            # Contrastive models
            case "text_infonce":
                from neurovlm.resources.loaders import _proj_head_text_infonce
                return _proj_head_text_infonce()
            case "image_infonce":
                from neurovlm.resources.loaders import _proj_head_image_infonce
                return _proj_head_image_infonce()
            # MSE text-to-brain model
            case "text_mse":
                from neurovlm.resources.loaders import _proj_head_text_mse
...
def _load_resolved_model(spec: ModelSpec):
    ...
    if spec.loader is ModelLoader.MLP_NEURO_QFORMER:
        from neurovlm.resources.loaders import _load_neuro_qformer
    ...
    if spec.loader is ModelLoader.MLP_NEURO_ADAPTER:
        from neurovlm.resources.loaders import _load_neuro_adapter
