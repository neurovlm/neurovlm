...
def generate_train_data_from_brain(
    ...
    for i in tqdm(range(len(pmids)), desc="Processing papers"):
        ...
        try:
            # Search concepts
            concepts_result = nvlm.brain(brain_vector).to_text(datasets=["cogatlas"], method="contrastive")
            concepts_df = concepts_result.top_k(k=k_concepts)
            top_concepts = concepts_df["title"].tolist()

            # Search disorders
            disorders_result = nvlm.brain(brain_vector).to_text(datasets=["cogatlas_disorder"], method="contrastive")
            disorders_df = disorders_result.top_k(k=k_disorders)
            top_disorders = disorders_df["title"].tolist()

            # Search tasks
            tasks_result = nvlm.brain(brain_vector).to_text(datasets=["cogatlas_task"], method="contrastive")
...
def generate_train_data_from_text(
    ...
    for i in tqdm(range(len(pmids)), desc="Processing papers"):
        ...
        try:
            ...
            concepts_result = nvlm.text(text_embedding).to_text(datasets=["cogatlas"], method="contrastive")
            concepts_df = concepts_result.top_k(k=k_concepts)
            top_concepts = concepts_df["title"].tolist()

            # Search disorders
            disorders_result = nvlm.text(text_embedding).to_text(datasets=["cogatlas_disorder"], method="contrastive")
            disorders_df = disorders_result.top_k(k=k_disorders)
            top_disorders = disorders_df["title"].tolist()

            # Search tasks
            tasks_result = nvlm.text(text_embedding).to_text(datasets=["cogatlas_task"], method="contrastive")
            tasks_df = tasks_result.top_k(k=k_tasks)
            top_tasks = tasks_df["title"].tolist()
