...
def run_epoch(autoencoder, text_projector, loader, text_cache, optimizer, device, loss_cfg, *, train: bool, max_batches: int | None = None):
    autoencoder.eval()
    ...
    for step, batch in enumerate(loader):
        ...
        with torch.no_grad():
            metric_rows.append(generation_metrics(torch.sigmoid(pred.detach().cpu()), target.detach().cpu()))
...
def train_from_config(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    train_loader = DataLoader(train_ds, batch_size=int(cfg.get("batch_size", 4)), shuffle=True, num_workers=int(cfg.get("num_workers", 0)), collate_fn=collator)
    val_loader = DataLoader(val_ds, batch_size=int(cfg.get("batch_size", 4)), shuffle=False, num_workers=int(cfg.get("num_workers", 0)), collate_fn=collator)
    ...
    for epoch in range(1, int(cfg.get("epochs", 3)) + 1):
        train_metrics = run_epoch(autoencoder, text_projector, train_loader, text_cache, optimizer, device, loss_cfg, train=True, max_batches=cfg.get("max_train_batches"))
        val_metrics = run_epoch(autoencoder, text_projector, val_loader, text_cache, optimizer, device, loss_cfg, train=False, max_batches=cfg.get("max_val_batches"))
