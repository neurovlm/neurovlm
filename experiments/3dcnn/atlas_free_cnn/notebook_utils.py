...
def _cache_embeddings_and_ids(payload: Any) -> tuple[Any, set[str]]:
    import torch
    ...
    if isinstance(payload, dict) and torch.is_tensor(payload.get("embeddings")):
        ...
        return payload["embeddings"].float(), text_ids
    if isinstance(payload, dict):
        for key in ["processed_embedding_by_text_id", "embedding_by_text_id", "text_id_to_embedding", "embeddings_by_text_id"]:
            value = payload.get(key)
            if isinstance(value, dict) and value:
                return torch.stack([torch.as_tensor(v, dtype=torch.float32) for v in value.values()]), {str(k) for k in value}
        for key in ["processed_embedding_by_text", "embedding_by_text", "text_to_embedding", "embeddings_by_text"]:
            value = payload.get(key)
            if isinstance(value, dict) and value:
                return torch.stack([torch.as_tensor(v, dtype=torch.float32) for v in value.values()]), set()
        ...
        if isinstance(records, list) and records:
            ...
            if vectors:
                return torch.stack(vectors), text_ids
    if isinstance(payload, dict) and payload and all(torch.is_tensor(v) for v in payload.values()):
        return torch.stack([v.float() for v in payload.values()]), set()
    raise TypeError("Unsupported normalized SPECTER2 cache format")
...
def validate_normalized_specter_cache(
    ...
    *,
    ...
    embeddings, text_ids = _cache_embeddings_and_ids(payload)
    ...
    return {"stats": stats, "metadata": payload.get("metadata", {}) if isinstance(payload, dict) else {}}
